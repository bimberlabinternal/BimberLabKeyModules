...
select
  ...
  a.name,
  a.ref_nt_id,
  a.ref_nt_id.name as refRT,
  a.rowid as ref_aa_id,
  locate(p.sequence, a.sequence) as start,
  (locate(p.sequence, a.sequence) +  length(p.sequence) - 1) as stop,
  (a.ref_nt_id.name || ' ' || a.name || ': ' || cast(locate(p.sequence, a.sequence) as varchar) || '-' || cast((locate(p.sequence, a.sequence) +  length(p.sequence) - 1) as varchar)) as peptidePosition
