...
public class MhcMigration
{
    private static final Logger _log = LogManager.getLogger(MhcMigration.class);

    private final String remoteServerFolder;
    private final String remoteConnectionName;

    private final User user;
    private final Container target;

    public MhcMigration(Container c, User u, String remoteConnectionName, String remoteServerFolder)
    {
        this.target = c;
        this.user = u;
    ...
    {
        DataIntegrationService.RemoteConnection rc = DataIntegrationService.get().getRemoteConnection(remoteConnectionName, target, _log);
    ...
    private void createLibraryMembers()
    {
        ...
        final UserSchema us = QueryService.get().getUserSchema(user, target, "sequenceanalysis");
        ...
        try
        {
            ...
            srr.getRowset().forEach(rd -> {
                ...
                try
                {
                    ...
                    List<Map<String, Object>> created = ti.getUpdateService().insertRows(user, target, Arrays.asList(toCreate), bve, null, null);
                    if (bve.hasErrors())
                    {
    ...
    private void createLibraries()
    {
        ...
        try
        {
            final TableInfo libraryTable = QueryService.get().getUserSchema(user, target, "sequenceanalysis").getTable("reference_libraries");
            ...
            srr.getRowset().forEach(rd -> {
                ...
                Container targetContainer = remoteWorkbook == null ? target : workbookMap.get(remoteWorkbook);
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        BatchValidationException bve = new BatchValidationException();
                        List<Map<String, Object>> created = libraryTable.getUpdateService().insertRows(user, target, Arrays.asList(toCreate), bve, null, null);
                        if (bve.hasErrors())
                        {
                            throw new RuntimeException(bve);
                        }
    ...
    private void createOutputFiles()
    {
        ...
        try
        {
            final TableInfo outputTable = QueryService.get().getUserSchema(user, target, "sequenceanalysis").getTable("outputfiles");
            ...
            srr.getRowset().forEach(rd -> {
                ...
                Readset rs = SequenceAnalysisService.get().getReadset(localReadset, user);
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        BatchValidationException bve = new BatchValidationException();
                        List<Map<String, Object>> created = outputTable.getUpdateService().insertRows(user, target, Arrays.asList(toCreate), bve, null, null);
                        if (bve.hasErrors())
                        {
                            throw new RuntimeException(bve);
                        }
    ...
    private void createAnalyses()
    {
        ...
        try
        {
            final TableInfo analysisTable = QueryService.get().getUserSchema(user, target, "sequenceanalysis").getTable("sequence_analyses");
            ...
            srr.getRowset().forEach(rd -> {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        BatchValidationException bve = new BatchValidationException();
                        List<Map<String, Object>> created = analysisTable.getUpdateService().insertRows(user, target, Arrays.asList(toCreate), bve, null, null);
                        if (bve.hasErrors())
                        {
                            throw new RuntimeException(bve);
                        }
    ...
    private void createReaddata()
    {
        ...
        try
        {
            final TableInfo readdataTable = QueryService.get().getUserSchema(user, target, "sequenceanalysis").getTable("readdata");
            ...
            srr.getRowset().forEach(rd -> {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        BatchValidationException bve = new BatchValidationException();
                        List<Map<String, Object>> created = readdataTable.getUpdateService().insertRows(user, target, Arrays.asList(toCreate), bve, null, null);
                        if (bve.hasErrors())
                        {
                            throw new RuntimeException(bve);
                        }
            ...
            srr.getRowset().forEach(rs -> {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        BatchValidationException bve = new BatchValidationException();
                        List<Map<String, Object>> created = readsetTable.getUpdateService().insertRows(user, target, Arrays.asList(toCreate), bve, null, null);
                        if (bve.hasErrors())
                        {
                            throw new RuntimeException(bve);
                        }
        ...
        try
        {
            ...
            srr.getRowset().forEach(pj -> {
                ...
                else
                {
                    ...
                    toCreate = Table.insert(user, ti, toCreate);
    ...
    private int createExpRun(int remoteId, Container c, String name, int localJobId) throws Exception
    {
        ...
        {
            ExpRun ret = ExperimentService.get().createRunForProvenanceRecording(c, user, new RecordedActionSet(), name, localJobId);
    ...
    private void createWorkbooks()
    {
        ...
        try
        {
            TableInfo containers = QueryService.get().getUserSchema(user, target, "core").getTable("containers");
            ...
            srr.getRowset().forEach(wb -> {
                ...
                else
                {
                    ...
                    Container workbook = ContainerManager.createContainer(target, null, localTitle, description, WorkbookContainerType.NAME, user);
                    workbookMap.put(Integer.parseInt(String.valueOf(wb.getValue("Name"))), workbook);
                }
            });
        }
