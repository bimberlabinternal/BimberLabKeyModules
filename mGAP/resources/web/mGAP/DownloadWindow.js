            ...
            success: function (results) {
                ...
                var toAdd = [{
                    html: 'Due to the large file size, the preferred option is to download using wget or curl on the command line, such as the exmaples below. Nonetheless, you also are able to paste the URLs into your browser and download through this way as well, although it will be slower and possibly not able to resume if your connection is disrupted.<br><br>' +
                            ...
                            '<pre>curl -C https://mgapdownload.ohsu.edu/' + releaseVcf + '<br>' +
                            'curl -C https://mgapdownload.ohsu.edu/' + releaseVcf + '.tbi</pre>' +
                            (sitesOnlyVcf ?
                            'or a VCF without genotypes (considerably smaller):<br>' +
                            '<pre>curl -C https://mgapdownload.ohsu.edu/' + sitesOnlyVcf + '<br>' +
                            'curl -C https://mgapdownload.ohsu.edu/' + sitesOnlyVcf + '.tbi</pre>' : '') +
                            'and genome:<br>' +
                            '<pre>curl -C https://mgapdownload.ohsu.edu/genomes/' + urlFasta + '<br>' +
                            'curl -C https://mgapdownload.ohsu.edu/genomes/' + urlFasta + '.fai<br>' +
                            'curl -C https://mgapdownload.ohsu.edu/genomes/' + urlFasta.replace(/fasta$/, 'dict') + '</pre>'
                },{
