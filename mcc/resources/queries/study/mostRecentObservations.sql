...
FROM study.clinical_observations co
    WHERE
        ...
        co.category IN ('Medical History', 'Fertility Status', 'Infant History', 'Current Housing Status', 'Availability', 'Usage_Current', 'Usage_Future') AND
        co.date = (SELECT max(o.date) asMaxDate
                   FROM study.clinical_observations o
                   WHERE o.Id = co.Id AND
                         o.qcstate.publicdata = true AND
                         o.category IN ('Medical History', 'Fertility Status', 'Infant History', 'Current Housing Status', 'Availability', 'Usage_Current', 'Usage_Future') AND
